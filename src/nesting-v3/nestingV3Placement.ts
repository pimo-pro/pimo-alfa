...
import type { V3Piece, V3Placement, V3Sheet } from "./nestingV3Types";
import type { NestingV3Settings } from "./nestingV3Settings";
import { hasOverlap } from "./nestingV3Engine";

export function effectiveDims(piece: V3Piece): { w: number; h: number } {
...
export function isPlacementValid(
  ...
  const candidate: V3Placement = { pieceId, sheetIndex, xMm, yMm };
  const others = placements
    ...
    .filter((x): x is { pl: V3Placement; w: number; h: number } => x != null);

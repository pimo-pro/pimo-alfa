 ...
 * 1. `PI_PALETTE_OVERRIDES` — paleta Pi CI pura (`var(--ci-*)` / color-mix)
 * 2. `CI_SSOT_TOKEN_BRIDGE` — namespace `--ci-*` / escalas (não redefine tokens Alpha)
 * 3. `pimo-pi-token-overrides` — overrides de utilizador / editor (ganham sobre CI)
 *
 * Alpha nunca passa por este merge (ThemeTemplateContext sai cedo).
 * Botões (`PI_BUTTON_SYSTEM_TOKENS`) continuam aplicados — parte no Context.
 */
...
/** Devolve as trés camadas cruas (sem merge) para um modo. */

...
import { exportNestingV4ToCnc } from "./nestingV4Export";
import { runNestingV4AutoLayout } from "./nestingV4Engine";
import type { NestingV4State, V4Piece } from "./nestingV4Types";
import { DEFAULT_NESTING_V4_SETTINGS } from "./nestingV4Settings";
import { defaultSheetFromSettings } from "./nestingSheetsFactory";

function makeState(pieces: V4Piece[], placements: NestingV4State["placements"]): NestingV4State {
  const settings = { ...DEFAULT_NESTING_V4_SETTINGS };
  return {
...
function assertValidTcnExport(result: ReturnType<typeof exportNestingV4ToCnc>): void {
...
describe("exportNestingV4ToCnc", () => {
  it("export manual V3 gera TCN válido", () => {
    ...
    assertValidTcnExport(exportNestingV4ToCnc(state, "ManualV3"));
  });

  it("export após auto-layout industrial gera TCN válido", () => {
    const settings = { ...DEFAULT_NESTING_V4_SETTINGS, sheetWidthMm: 2800, sheetHeightMm: 2070 };
    ...
    const layout = runNestingV4AutoLayout(pieces, sheets, settings);
    const state: NestingV4State = {
    ...
    const exportResult = exportNestingV4ToCnc(state, "AutoV3");

...
export default function NestingV4StationSidebar() {
  const location = useLocation();
  const onNestingV4 = location.pathname === "/nesting_v4";
  ...
  return (
    ...
    >
      <Link
        to="/nesting_v4"
        title="Layout de Corte MANUAL (Nesting V4)"
        style={{
          ...industrialBtnStyle(onNestingV4),
          width: 40,
          height: 40,
          display: "grid",
          placeItems: "center",
      ...
      >
        <span style={{ fontSize: 8, fontWeight: 800, letterSpacing: 0.2 }}>V4</span>
      </Link>
      <Link
      ...
      {INDUSTRIAL_STATIONS.map((station) => {
        ...
        const active = !onNestingV4 && !onLca && location.pathname === path;
        return (
          <Link
            key={station}
            to={path}

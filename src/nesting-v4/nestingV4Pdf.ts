/**
 * Nesting V4 — PDF Técnico Completo
 *
 * Gera um PDF com:
...
import type { NestingV4State, V4Piece, V4Placement, V4Sheet } from "./nestingV4Types";
import { rotateHoles } from "./nestingV4Engine";
import { calcSheetUtilization } from "./nestingV4Engine";
...
function effectiveDims(piece: V4Piece): { w: number; h: number } {
...
function addCoverPage(doc: jsPDF, state: NestingV4State, projectName: string): void {
  const [nr, ng, nb] = CLR_NAVY;
  doc.setFillColor(nr, ng, nb);
  ...
  doc.setFontSize(13);
  doc.setFont("helvetica", "normal");
  doc.text("Nesting V4 — Layout de Corte Manual", MARGIN, 32);
  ...
  doc.text("Gerado pelo PIMO Criativo — Nesting V4 — Layout Manual de Corte", A4_W / 2, A4_H - 8, { align: "center" });
  doc.setTextColor(0, 0, 0);
}
...
function addPieceDetailPage(
  doc: jsPDF,
  piece: V4Piece,
  placement: V4Placement | undefined,
  pageNum: number
): void {
...
/**
 * Gera o PDF técnico completo do Nesting V4.
 * Retorna um Blob para download.
 */
export function generateNestingV4Pdf(
 ...
 */
export async function downloadNestingV4LayoutProPdf(
  state: NestingV4State,
  projectName = "Projeto"
): Promise<void> {
  const { prepareNestingV4IndustrialLayout } = await import("./nestingV4Export");
 ...
 */
export async function downloadNestingV4Pdf(state: NestingV4State, projectName = "Projeto"): Promise<void> {
  await downloadNestingV4LayoutProPdf(state, projectName);
  const { downloadNestingV4ArmazemPdf } = await import("./nestingV4Export");
  await downloadNestingV4ArmazemPdf(state, projectName);
}

/**
 * Nesting V4 — Etiquetas auxiliares da estação de layout.
 *
 * SSOT de produção: UnifiedEtiquetaEngine + LabelSystemV5 (`core/etiquetas/ssot`).
 * Este PDF é auxiliar da estação (peça + folha + posição V3); não substitui UEE.
 */

import jsPDF from "jspdf";
import qrcode from "qrcode-generator";
import type { NestingV4State, V4Piece, V4Placement } from "./nestingV4Types";
...
function buildQrPayload(piece: V4Piece, placement: V4Placement, projectName: string): string {
 ...
 */
export function generateNestingV4Labels(
 ...
 */
export function downloadNestingV4Labels(state: NestingV4State, projectName = "Projeto"): void {
  const blob = generateNestingV4Labels(state, projectName);

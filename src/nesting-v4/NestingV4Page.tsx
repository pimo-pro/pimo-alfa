/**
 * Nesting V4 — Página de Layout Manual de Corte
 *
 * Funcionalidades:
...
import { useNestingV4 } from "./useNestingV4";
import { calcSheetUtilization, calcWastePercent, rotateHoles } from "./nestingV4Engine";
import { downloadNestingV4Tcn, getV4ExportStats } from "./nestingV4Export";
import { downloadNestingV4Pdf } from "./nestingV4Pdf";
import { downloadNestingV4Labels } from "./nestingV4Labels";
import {
  beginIndustrialFileGeneration,
  endIndustrialFileGeneration,
} from "../core/fabrication/industrialGenerationSuspend";
import type { V4Piece, V4Placement } from "./nestingV4Types";
import type { CutPiece } from "../core/cutlayout/cutLayoutTypes";
import { Icon } from "../components/icons/Icon";
import { useProject } from "../context/useProject";
import { convertProjectToV4Pieces } from "./utils/convertProjectToV4Pieces";
import { IndustrialThreeColumnLayout } from "@/industrial/ui/layouts/IndustrialThreeColumnLayout";
import NestingV4StationSidebar from "./components/NestingV4StationSidebar";
import NestingV4SettingsPanel from "./components/NestingV4SettingsPanel";
import type { V4Sheet } from "./nestingV4Types";

// ── Props ─────────────────────────────────────────────────────────────────────

interface NestingV4PageProps {
  initialCutPieces?: CutPiece[];
  initialPieces?: V4Piece[];
  projectName?: string;
  projectId?: string;
  onClose?: () => void;
...
function effectiveDims(piece: V4Piece) {
...
function PropertiesPanel({ piece, placement, onRotate, onReturn, onMoveSheet, sheetCount }:{
  piece: V4Piece; placement?: V4Placement;
...
function PieceSidebar({ unplacedIds, pieces, sheets, placements, selectedId, onSelect, onDragStart, onRemove, onRotate, onFocus, onSelectSheet }:{
  unplacedIds: string[]; pieces: V4Piece[]; sheets: V4Sheet[];
  placements: V4Placement[]; selectedId: string | null;
...
function PieceListItem({ piece, selected, onSelect, onDragStart, onRotate, onRemove }: {
  piece: V4Piece; selected: boolean;
...
interface SheetCanvasProps {
  ...
  onPiecePointerDown: (e: PointerEvent<SVGGElement>, id: string, pl: V4Placement) => void;
  onRotate: (id: string) => void;
...
function SheetCanvas({ sheet, sheetIndex, pieces, placements, kerfMm: _kerfMm, marginMm = 0, scale, selectedId, onSelect, onPiecePointerDown, onRotate, onReturn, onMoveToSheet, sheetCount, showGrainHatch = true, showWasteOverlay = true, wastePercent }: SheetCanvasProps) {
  ...
  return (
    <svg width={W} height={H} style={{ display: "block", userSelect: "none" }}
      ...
      {myPlacements.map((pl) => {
        ...
        return (
          <g key={piece.id}
            ...
            {/* Contorno / kerf visual */}
            <rect x={px} y={py} width={pw} height={ph} fill="none"
              stroke={darken(piece.color, 50)} strokeWidth={0.5} opacity={0.35} rx={1.5}/>
            {/* Grain / hachura */}
            {showGrainHatch && (
              piece.industrialGrainCode === "YY" ? (
                <>
                  {Array.from({ length: Math.max(2, Math.floor(pw / 8)) }).map((_, i) => (
                    <line
                      key={`gy-${i}`}
                      x1={px + 4 + i * 8}
                      y1={py + 2}
            ...
            {/* Grain badge */}
            {piece.industrialGrainCode && pw > 36 && (
              <text x={px + 3} y={py + 3} dominantBaseline="hanging" fontSize={7}
                fill={darken(piece.color, 90)} opacity={0.7} style={{ pointerEvents: "none" }}>
                {piece.industrialGrainCode}
              </text>
            )}
            {/* Holes */}
            {holes.map((h, hi) => (
      ...
      {showWasteOverlay && typeof wastePercent === "number" && (
        <text x={W - 6} y={H - 6} textAnchor="end" fontSize={10} fill="rgba(0,0,0,0.45)" style={{ pointerEvents: "none" }}>
          Desperdício {wastePercent.toFixed(1)}%
        </text>
      )}
    </svg>
  );
...
type NestingV4LocationState = {
  openNestingV4?: boolean;
  pieces?: V4Piece[];
  projectId?: string;
  projectName?: string;
};
...
export default function NestingV4Page({
  initialCutPieces = [],
  initialPieces,
  projectName = "Projeto",
  projectId,
  ...
  const locationState = (location.state ?? null) as NestingV4LocationState | null;
  ...
  const stats       = getV4ExportStats(state);
  ...
  const handlePiecePointerDown = useCallback((e: PointerEvent<SVGGElement>, id: string, pl: V4Placement) => {
  ...
  const handleGenerateAll = useCallback(async () => {
    ...
    try {
      ...
      await downloadNestingV4Pdf(state, resolvedProjectName);
      await new Promise((r) => setTimeout(r, 100));
      downloadNestingV4Tcn(state, resolvedProjectName);
      await new Promise((r) => setTimeout(r, 100));
      downloadNestingV4Labels(state, resolvedProjectName);
    } finally {
      endIndustrialFileGeneration();
      setGenerating(false);
    }
  ...
  const workspace = (
    ...
    >
      ...
      <div style={{ height:TOOLBAR_H, display:"flex", alignItems:"center", gap:6, padding:"0 12px", background:C.surface, borderBottom:`1px solid ${C.border}`, flexShrink:0, zIndex:10 }}>
        ...
        <span style={{ fontSize:13, fontWeight:700, color:C.text }}>Nesting V4</span>
        ...
        <button type="button" onClick={() => {
          beginIndustrialFileGeneration();
          try { downloadNestingV4Labels(state, resolvedProjectName); }
            ...
            <div style={{ position:"absolute", transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`, transformOrigin:"0 0" }}>
              {canvasView === "overview" ? (
                  ...
                  {state.sheets.map((sheet, si) => (
                    <div key={si} style={{ cursor: "pointer" }} onClick={() => { setActiveSheet(si); setCanvasView("single"); }}>
                      ...
                      <SheetCanvas
                        ...
                        showGrainHatch={state.settings.showGrainHatch}
                        showWasteOverlay={state.settings.showWasteOverlay}
                        wastePercent={calcWastePercent(si, sheet, state.placements, state.pieces)}
                      />
                    </div>
                  ))}
                </div>
                ...
                <SheetCanvas
                  ...
                  showGrainHatch={state.settings.showGrainHatch}
                  showWasteOverlay={state.settings.showWasteOverlay}
                  wastePercent={calcWastePercent(state.activeSheetIndex, activeSheet, state.placements, state.pieces)}
                />
              ) : null}
            </div>
  ...
  if (layout === "station") {
    return (
      <IndustrialThreeColumnLayout
        ...
        description={`Nesting V4 — ${resolvedProjectName}`}
        sidebarOpen={false}
        leftLeft={<NestingV4StationSidebar />}
        left={leftPanel}
        right={workspace}
      />
    );

...
export default function DrawersAdminHubPage() {
  ...
  /** Com Modelo B activo (default de produto), abre directamente no catálogo europeu. */
  ...
  const ensureDocsSample = () => {
    ...
    if (modeloAActive) {
      setDocsError("Desactive o Modelo A para gerar a documentação de amostra do Modelo B.");
      return null;
    }
    setDocsError(null);
    ...
    if (!result.docs) {
      setDocsError("generateEuropeanDrawer não devolveu docs.");
      return null;
    }
    ...
    <div style={{ ...adminPageShellStyle, maxWidth: 1200 }}>
      ...
      {section === "modelo-b" ? (
        <>
          ...
          <Panel
            title="Documentação Industrial (Modelo B)"
            description="Camada adicional Fase 11 — ficha técnica + PDF multi-páginas (estrutura). Não substitui o PDF industrial existente."
          >
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
              ...
              >
                Ver PDF multi-páginas (estrutura)
              </button>
              {docsSample?.report ? (
                <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                  {docsSample.report.status} — {docsSample.report.piecesDocumented} peças ×{" "}
                  {docsSample.report.holesDocumented} furos — {docsSample.report.logicalPages} págs
                </span>
              ) : null}
            </div>
            {docsError ? (
          ...
          <Panel
            ...
            description="Camada adicional Fase 12 — vistas industriais + estrutura DXF em memoria. Nao altera viewer 3D nem PDF industrial."
          >
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
              ...
              {dxfSample?.report ? (
                <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                  {dxfSample.report.status} — {dxfSample.report.contourCount} contornos ×{" "}
                  {dxfSample.report.holeEntityCount} furos — {dxfSample.report.viewCount} vistas
                </span>
              ) : null}
            </div>
            {dxfError ? (
          ...
          <Panel
            ...
            description="Fase 16 — gera ficheiros .dxf reais a partir de result.dxf (em memoria). No browser faz download; em Node escreve em exports/dxf/european/."
          >
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
              <button
                type="button"
              ...
              >
                {dxfFileBusy ? "A gerar…" : "Gerar DXF (ficheiros)"}
              </button>
              {dxfFileReport ? (
                <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                  {dxfFileReport.status} — {dxfFileReport.files.length} ficheiros ×{" "}
                  {dxfFileReport.totalBytes} bytes — {dxfFileReport.outputDir}
                </span>
              ) : null}
            </div>
            {dxfFileReport ? (
              <ul style={{ marginTop: 10, paddingLeft: 18, fontSize: 11, color: "var(--text-muted)", lineHeight: 1.45 }}>
                {dxfFileReport.files.map((f) => (
                  <li key={f.fileName}>
                    {f.fileName} — {f.relativePath} ({f.byteLength} B, {f.pieceCode})
                  </li>
                ))}
              </ul>
            ) : null}
          ...
          <Panel
            ...
            description="Fase 17 — gera ficheiros CNC físicos a partir de geometry + holes + dxf. Formatos: cnc/xml/mpr/cix/bpp. No browser faz download; em Node escreve em exports/cnc/european/."
          >
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
              ...
              >
                {cncFileBusy ? "A gerar…" : "Gerar ficheiros CNC"}
              </button>
              {cncFileReport ? (
                <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                  {cncFileReport.status} — {cncFileReport.files.length} ficheiros — CUT{" "}
                  {cncFileReport.totalCutOps} / DRILL {cncFileReport.totalDrillOps} ×{" "}
            ...
            {cncFileReport ? (
              <ul style={{ marginTop: 10, paddingLeft: 18, fontSize: 11, color: "var(--text-muted)", lineHeight: 1.45 }}>
                {cncFileReport.files.map((f) => (
                  <li key={f.fileName}>
                    {f.fileName} — {f.relativePath} (CUT {f.cutCount}, DRILL {f.drillCount}, {f.byteLength} B)
                  </li>
                ))}
              </ul>
            ) : null}
          ...
          <Panel
            ...
            description="Camada adicional Fase 13 — medidas internas, aberturas, gaps, remates e roda-pe. Integrado com DXF e vistas tecnicas."
          >
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
              ...
              {overlaySample?.report ? (
                <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                  {overlaySample.report.status} — {overlaySample.report.aberturaCount} aberturas ×{" "}
                  {overlaySample.report.gapCount} gaps — {overlaySample.report.remateCount} remates
                </span>
              ) : null}
            </div>
            {overlayError ? (
          ...
          <Panel
            ...
            description="Camada adicional Fase 14 — geracao automatica de notes industriais (B.v3.14). Nao altera pipeline industrial."
          >
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
              ...
              {releaseSample ? (
                <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                  {releaseSample.version} — {releaseSample.report.status} ×{" "}
                  {releaseSample.eventsCollected} eventos — {releaseSample.author}
                </span>
              ) : null}
            </div>
            {releaseError ? (
          ...
          <Panel
            ...
            description="Fase 18 — motor de custo industrial (materiais, ops, CNC, montagem, mão de obra, overhead, margem). Camada somente-leitura."
          >
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
              <button
                type="button"
              ...
              {pricingSample ? (
                <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                  {pricingSample.report.status} — custo {pricingSample.totals.costIndustrial}{" "}
                  {pricingSample.currency} — preço {pricingSample.totals.priceFinal}{" "}
                  {pricingSample.currency} — margem{" "}
            ...
            {pricingSample ? (
              <ul style={{ marginTop: 10, paddingLeft: 18, fontSize: 11, color: "var(--text-muted)", lineHeight: 1.45 }}>
                ...
                <li>
                  Custo por módulo: {pricingSample.totals.costPerModule} {pricingSample.currency}
                </li>
                <li>
                ...
                <li>
                  Preço final: {pricingSample.totals.priceFinal} {pricingSample.currency} (gaveta{" "}
                  {pricingSample.totals.pricePerDrawer} / módulo {pricingSample.totals.pricePerModule})
                </li>
              </ul>
            ) : null}
          </Panel>
      ...
      {section === "kitchen" ? (
        <Panel
          ...
          description="Fase 15 / PIMO.PRO-V5 Fase 10 — biblioteca documental de módulos, frentes, portas, gavetas Modelo B, remates e roda-pé."
        >
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
            ...
            {kitchenLib ? (
              <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                {kitchenLib.version} — {kitchenLib.report.status} — {kitchenLib.report.moduleCount} módulos ×{" "}
                {kitchenLib.report.frontCount} frentes — {kitchenLib.report.doorCount} portas
              </span>
            ) : null}
          </div>
          {kitchenError ? (

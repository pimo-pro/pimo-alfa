...
export function scorePlacement(
  ...
  const tightnessBonus = tightnessVal * (1.2 + lateFactor * 0.6);
  ...
  const isolationPenalty = tightnessVal === 0 ? (0.45 + lateFactor * 0.5) : 0;

  // Fase 3: islandPenalty — penaliza peças que criam "ilhas" (tightnessScore < 0.15).
  const islandPenalty = (tightnessVal < 0.25 ? 0.55 : 0) * (1 + lateFactor);

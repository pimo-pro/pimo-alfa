...
describe("centralPricingConfig", () => {
  ...
  it("orcamentosDefaultsFromCentral activa Desperdício/Serragem; MO off por defeito", () => {
    ...
    expect(orc.custosIndustriais.enableMaoDeObra).toBe(false);
    expect(orc.custosIndustriais.valorHoraMaquina).toBe(0);
    expect(orc.custosIndustriais.serragemEurPorM2).toBeGreaterThan(0);
    // Montagem/gavetas intacta (canal separado).
    expect(orc.custosIndustriais.custoMontagemPorPeca).toBe(15);
  });
  ...
  it("Mão de obra financeira não herda montagem nem força 35", () => {
    const n = normalizeCentralPricing({
      version: 2,

/**
 * finalAssembler.ts — Consolida artefactos PIMO.PRO-V5 sem alterar componentes.
 */
...
export type FinalAssembledRelease = {
  ...
  /** Conteúdos DXF em memória (não escreve disco). */
  dxfFiles: Array<{ fileName: string; pieceCode: string }>;
  /** Conteúdos CNC em memória (não escreve disco). */
...
export type AssembleFinalReleaseOptions = {
  /** Se false, não gera amostra Modelo B (apenas library). Default true. */
  includeModeloBSample?: boolean;
  /** Coloca módulos demo no planner. Default true. */
  seedPlannerModules?: boolean;
};

/**
 * Consolida Modelo B + Kitchen + Planner + docs/pricing/DXF/CNC referências.
 * Não muta geometry/furos/cutlist existentes — apenas lê e agrega.
 */

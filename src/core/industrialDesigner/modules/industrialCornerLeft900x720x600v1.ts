/**
 * Módulo industrial built-in: Armário Canto Esquerda 900×720×600 mm.
 */
...
} from "./industrialCornerLeftConstants";
...
export type BuildIndustrialCornerLeft900Options = {
...
export function buildIndustrialCornerLeft900x720x600DesignBox(
  options: BuildIndustrialCornerLeft900Options = {}
): IndustrialDesignBox {
  ...
  const { panels, constraints } = buildCornerLeftIndustrialPanels({
    boxId: INDUSTRIAL_CORNER_LEFT_900_MODULE_ID,
  ...
  box = applyAllIndustrialCornerDrillingRules(box, {
    ...
    hingeSide: "left",
    shelfTechnicalMargins: INDUSTRIAL_CORNER_LEFT_SHELF_TECHNICAL_MARGINS,
    layout,
    cornerSide: "left",
  });
  ...
  if (blocking.length > 0) {
    throw new Error(
      `Módulo canto industrial esquerda inválido: ${blocking.map((i) => i.message).join("; ")}`
    );
  }
...
export function buildIndustrialCornerLeft900ModelRecord(
  ...
  const projectCtx: DesignDrillExportProjectContext = project ?? {
    projectName: "MODULO_INDUSTRIAL_CANTO_ESQUERDA",
    boxes: [],
  ...
  return {
    ...
    metadata: {
      ...
      moduleKind: "industrial-corner-left-900x720x600",
...
export function registerIndustrialCornerLeft900x720x600Module(): CustomIndustrialModelRecord {
  const existing = getBuiltinIndustrialModel(INDUSTRIAL_CORNER_LEFT_900_MODULE_ID);
  if (existing) return existing;

  const record = buildIndustrialCornerLeft900ModelRecord();

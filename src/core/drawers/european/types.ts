...
/** Codigo comercial da altura (ex.: N, M, K, F, H / D, Cé). */
...
export type DrawerHeightProfile = {
  /** Codigo de catalogo (N/M/Ké). Vazio se o sistema so usa mm. */
 ...
 * Industrial Modelo B: frente madeira + laterais/costa/fundo madeira + corrediça Hettich.
 */
...
export type DrawerCutlistItem = {
  id: string;
  nome: string;
  /** Código industrial (gav_fren, gav_lat_dir, —). */
  codigo?: string;
  quantidade: number;
...
export type EuropeanDrawerBoxInput = {
  ...
  gavetas?: number;
  material?: string;
  /** Profundidade útil interna (mm) — preferida para seleção Hettich. */
...
export type EuropeanDrawerResult = {
  ...
  errors: string[];
  warnings: string[];
  /** Descrições de auto-fix disponíveis (sem funções — serializável). */
  ...
  /** Documentação industrial completa (Fase 11) — camada somente-leitura. */
  docs?: import("./docs").EuropeanIndustrialDocs;
  /** Export DXF em memória (Fase 12). */
  dxf?: import("./dxf").EuropeanDXFExport;
  /** Modo de desenho técnico / vistas industriais (Fase 12). */
  ...
  /** Custo industrial (Fase 18) — camada somente-leitura. */

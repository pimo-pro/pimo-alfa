...
export function validateCutlist(
  ...
  for (const tipo of REQUIRED_WOOD_TIPOS) {
    if (!woodTipos.has(tipo)) {
      result.errors.push(
        euError(EU_ERROR_CODES.CUT_WOOD, `Cutlist sem peça obrigatéria "${tipo}".`, `cutlist.${tipo}`)
      );
    }
  }
  ...
  for (const item of cutlist) {
    ...
    if (item.kind === "wood") {
      if (item.larguraMm <= 0 || item.alturaMm <= 0 || item.espessuraMm <= 0) {
        result.errors.push(
          euError(EU_ERROR_CODES.CUT_DIM, `Dimensóo <= 0 em peça madeira "${item.nome}".`, `cutlist.${item.id}`)
        );
      }
      if (!item.codigo) {
        result.errors.push(

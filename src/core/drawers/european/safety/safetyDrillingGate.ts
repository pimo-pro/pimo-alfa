/**
 * safetyDrillingGate.ts — Bloqueia furos industriais inválidos.
 */
...
/** Margem: bloquear só claramente fora (coordenada negativa ou além da peça). */
...
function pieceBounds(
  ...
  // Apenas frente/fundo (furos gerados localmente). Laterais/costa/módulo: eixos Modelo A — não forçar.
...
export function runSafetyDrillingGate(
  ...
  holes.forEach((h, idx) => {
    ...
    if (!Number.isFinite(h.diameter) || h.diameter < 0) {
      ...
      // Alguns furos Modelo A / marcadores industriais emitem —=0 — aviso, nao bloqueio
      warnings.push(

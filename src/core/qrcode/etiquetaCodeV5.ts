...
/** Sem boxId: agrupa por nome da peça (evita NUM_CAIXA = 0 para tudo). */
...
/**
 * Mapa itemKey → total de peças na mesma folha/caixa (NUM_CAIXA).
 *
 * Com placements (nesting): sheetIndex; sem match → fallback boxId; senão 0.
 * Sem placements: boxId; sem boxId → agrupa por nome.
 *
 * Futuro (PIMO-TRACK / peças MANUAL): peças manuais isoladas podem forçar NUM_CAIXA = 1
 * quando existir tipo/metadata MANUAL — não implementado nesta fase.
 */

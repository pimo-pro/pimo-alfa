import type { RematePiece, RematePieceTipo } from "./rematePieceTypes";
import { RODAPE_MAX_LENGTH_MM } from "./remateTypes";

export type RemateMergeGroup = {
  id: string;
  remateIds: string[];
  faceKind: RematePieceTipo;
  spanMm: number;
};

const TOUCH_TOLERANCE_MM = 8;
const ALIGN_TOLERANCE_MM = 12;

function sameTipo(a: RematePiece, b: RematePiece): boolean {
  return a.tipo === b.tipo && a.tipo !== "L" && a.tipo !== "RODAPE_L";
}

function axisKey(remate: RematePiece): "x" | "y" | "z" {
  if (remate.tipo === "RODAPE" || remate.tipo === "RODAPE_L") return "x";
  if (remate.tipo === "DIR" || remate.tipo === "ESQ") return "y";
  if (remate.tipo === "CIMA" || remate.tipo === "BAIXO") return "x";
  return "x";
}

function spanOnAxis(remate: RematePiece, axis: "x" | "y" | "z"): number {
  const { width: w, height: h, depth: d } = remate;
  if (axis === "x") return w;
  if (axis === "y") return h;
  return d;
}

export function computeRemateVisualMergeGroups(remates: RematePiece[]): RemateMergeGroup[] {
  const groups: RemateMergeGroup[] = [];
  const used = new Set<string>();

  for (let i = 0; i < remates.length; i++) {
    const a = remates[i]!;
    if (used.has(a.id) || a.tipo === "L" || a.tipo === "RODAPE_L") continue;

    const axis = axisKey(a);
    const cluster: RematePiece[] = [a];
    used.add(a.id);

    for (let j = i + 1; j < remates.length; j++) {
      const b = remates[j]!;
      if (used.has(b.id)) continue;
      if (!sameTipo(a, b)) continue;
      if (axisKey(b) !== axis) continue;
      const touchesCluster = cluster.some((member) => areRematesTouching(member, b));
      if (!touchesCluster) continue;
      cluster.push(b);
      used.add(b.id);
    }

    if (cluster.length < 2) continue;

    const spanMm = cluster.reduce((sum, r) => sum + spanOnAxis(r, axis), 0);
    if (cluster[0]!.tipo === "RODAPE" && spanMm > RODAPE_MAX_LENGTH_MM) continue;

    const groupId = `merge-${cluster.map((r) => r.id).join("-")}`;
    groups.push({
      id: groupId,
      remateIds: cluster.map((r) => r.id),
      faceKind: cluster[0]!.tipo,
      spanMm,
    });
  }

  return groups;
}

export function remateIdsInMergeGroup(groups: RemateMergeGroup[]): Set<string> {
  const set = new Set<string>();
  for (const g of groups) {
    if (g.remateIds.length >= 2) {
      g.remateIds.forEach((id) => set.add(id));
    }
  }
  return set;
}

export function areRematesTouching(a: RematePiece, b: RematePiece): boolean {
  if (!sameTipo(a, b)) return false;
  const axis = axisKey(a);
  if (axisKey(b) !== axis) return false;
  const spanA = spanOnAxis(a, axis);
  const spanB = spanOnAxis(b, axis);
  const tx = Math.abs(a.position.xMm - b.position.xMm);
  const ty = Math.abs(a.position.yMm - b.position.yMm);
  const tz = Math.abs(a.position.zMm - b.position.zMm);
  const dist = Math.max(tx, ty, tz);
  return dist <= Math.max(spanA, spanB) + TOUCH_TOLERANCE_MM && dist <= ALIGN_TOLERANCE_MM + spanA + spanB;
}

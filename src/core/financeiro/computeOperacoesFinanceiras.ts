 ...
 * CNC = perímetro real — corte_cnc_metro (pricing.json).
 * Drill = furos reais — furo_cnc (exclui rasgos/grooves).
 */
...
export type OperacoesFinanceirasTarifas = {
  ...
  /** — /m de perímetro de corte CNC. */
  corteEurPorMetro: number;
  /** Legado — não usar para —; mantido para compat. */
...
/** Peça nestável se tem área + espessura. */
...
/** Perímetro de corte em metros (2 — (L+A) / 1000). */
...
/** Furos reais (exclui rasgos/grooves — cobrados só em ops avançadas em gavetas). */
 ...
 * Sem flat-fee por peça; sem rasgos fantasma.
 */

/**
 * Benchmark CutLayout — mono-espessura 19 mm (20–30 peças, sem costa 10 mm).
 *
 *   $env:CUTLAYOUT_BENCH_PHASE="after"
 *   npx vitest run src/validation/cutlayoutMono19Benchmark.test.ts
 */
...
import {
  applyMono19BenchmarkMaterials,
  buildMono19BenchmarkScenario,
  inferIndustrialPieceKind,
  MONO19_BENCH_NOME,
  MONO19_BENCH_PROJECT,
...
const BEFORE_JSON = join(OUT_DIR, "MONO19_CUTLAYOUT_BEFORE.json");
const AFTER_JSON = join(OUT_DIR, "MONO19_CUTLAYOUT_AFTER.json");
const MERGED_JSON = join(OUT_DIR, "MONO19_CUTLAYOUT_BENCHMARK.json");
const REPORT_TXT = join(OUT_DIR, "MONO19_CUTLAYOUT_BENCHMARK.txt");
const REPORT_MD = join(OUT_DIR, "MONO19_CUTLAYOUT_BENCHMARK.md");
const INPUT_MANIFEST = join(OUT_DIR, "MONO19_BENCH_INPUT.json");
...
async function runPhase(phase: Phase): Promise<BenchmarkPayload> {
  ...
  const { snap } = buildMono19BenchmarkScenario();
  ...
  const items = applyMono19BenchmarkMaterials(rawItems);
  const pieces = cutlistToPieces(items, {
    projectName: snap.projectName ?? MONO19_BENCH_PROJECT,
    boxes: snap.boxes,
  });
  ...
  const thicknesses = [...new Set(pieceManifest.map((p) => p.espessura_mm))].sort();

  await writeFile(
    INPUT_MANIFEST,
    JSON.stringify(
      {
        project: MONO19_BENCH_PROJECT,
        box: MONO19_BENCH_NOME,
        pieceCount: pieceManifest.length,
        materialIds,
        thicknessesMm: thicknesses,
        mono19: thicknesses.length === 1 && thicknesses[0] === 19,
  ...
  return {
    ...
    generatedAt: new Date().toISOString(),
    inputFixture: "src/validation/industrialPipelineTestHelpers.ts → buildMono19BenchmarkScenario()",
    scenario: {
      scenarioId: "MONO19_SPM_COZINHA",
      description: `${MONO19_BENCH_NOME} — mono 19 mm, 4 gavetas, 2 DIV, 4 SEP, sem costa, remate + rodapé`,
...
async function mergeReports(before: BenchmarkPayload, after: BenchmarkPayload): Promise<void> {
  ...
  const merged = {
    title: "CutLayout — Benchmark mono-espessura 19 mm (redução de chapas)",
    baselineCommit: BASELINE_COMMIT,
  ...
  const txt = [
    "MONO19 CUTLAYOUT BENCHMARK — ANTES vs DEPOIS",
    `Baseline: ${BASELINE_COMMIT}`,
    `Projecto: ${MONO19_BENCH_NOME} (${b.pieceCount} peças, mono 19 mm, material: ${b.materialIds.join(", ")})`,
    "",
    "RESUMO",
  ...
  const md = `# Benchmark CutLayout — mono 19 mm (${b.pieceCount} peças)

## Entrada
- Fixture: \`buildMono19BenchmarkScenario()\`
- Manifest: \`MONO19_BENCH_INPUT.json\`
- Mono-espessura: 19 mm | Material: ${b.materialIds.join(", ")}
...
describe("CutLayout mono-19 benchmark (20–30 peças)", () => {
  it(
    ...
    async () => {
      ...
      if (phase === "merge") {
        ...
        expect(await readFile(MERGED_JSON, "utf8")).toContain("MONO19");
        return;
      }
      ...
      expect(payload.scenario.pieceCount).toBeGreaterThanOrEqual(20);
      expect(payload.scenario.pieceCount).toBeLessThanOrEqual(30);
      expect(payload.scenario.materialIds).toEqual(["mdf_branco"]);
      expect(payload.scenario.pieceManifest.every((p) => p.espessura_mm === 19)).toBe(true);

/**
 * Atualiza notas/blurb/UI editorial de pimo-soon (UTF-8 via \\u).
 */
import fs from "node:fs";

const u = (...c) => String.fromCodePoint(...c);
const em = u(0x2014);
const sao = "S" + u(0xe3) + "o";
const evolucao = "evolu" + u(0xe7) + u(0xe3) + "o";
const crescimento = "crescimento";
const continuo = "cont" + u(0xed) + "nuo";
const pressao = "press" + u(0xe3) + "o";
...
export const PIMO_SOON_NOTAS: PimoSoonNote[] = [
  {
    id: "note-opcional",
    body: "Todas as fases s${u(0xe3)}o ${opcionais} e podem ser executadas sem ${pressao}.",
  },
  {
    id: "note-industrial",
  ...
  {
    id: "note-hub",
    body: "${sao} fases de ${evolucao} do Hub e do sistema documental.",
  },
  {
    id: "note-ativacao",
  ...
  {
    id: "note-estrategia",
    body: "Servem como base estrat${u(0xe9)}gica para o ${crescimento} ${continuo} do PIMO.",
  },
];
`,
  "utf8"
...
export function loadHubPimoSoon(): HubPimoSoonSnapshot {
  return {
    ...
    blurb:
      "Plano futuro do projeto: fases que poder${u(0xe3)}o ser desenvolvidas posteriormente, sem ${pressao} e sem impacto no pipeline industrial. Base estrat${u(0xe9)}gica para ${evolucao} cont${u(0xed)}nua do sistema.",
...
fs.writeFileSync(
  ...
  `/**
 * Sec${u(0xe7)}${u(0xe3)}o pimo-soon ${em} Plano Futuro (h${u(0xed)}brido A+C).
 */
...
const ui = fs.readFileSync("src/pages/documentacao/HubPimoSoonContent.tsx", "utf8");
console.log("notas", (notas.match(/id: "note-/g) || []).length);
console.log("estrategia", notas.includes("estrat"));
console.log("industrial combined", notas.includes("CNC, cutlist"));
console.log("FFFD", ui.includes("\uFFFD") || notas.includes("\uFFFD"));
console.log("header @tag", ui.includes("@{data.tag}") || ui.includes("@{data.tag}"));
console.log("title render", ui.includes("{data.title}"));

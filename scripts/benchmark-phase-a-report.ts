/**
 * Relatório comparativo Fase A — antes vs depois das melhorias cutlayout.
 * Lê PHASE_A_BEFORE_BENCHMARK.json e compara com execução actual.
 */
...
} from "../src/core/cutlayout/utils/cutLayoutUtils";
...
const BEFORE_JSON = join(OUT_DIR, "PHASE_A_BEFORE_BENCHMARK.json");
const OUT_JSON = join(OUT_DIR, "PHASE_A_COMPARISON_REPORT.json");
const OUT_TXT = join(OUT_DIR, "PHASE_A_COMPARISON_REPORT.txt");
...
const METRICS_DEPS = {
  estimateUsefulLeftover,
  computeSheetAdvancedMetrics: (sheet: Parameters<typeof computeSheetAdvancedMetrics>[0], placements: Parameters<typeof computeSheetAdvancedMetrics>[1]) =>
...
async function main() {
  ...
  const beforeRaw = JSON.parse(await readFile(BEFORE_JSON, "utf8")) as {
    scenarios: Array<{ scenario: string; v2: { totalSheets: number; wasteArea: number } }>;
  };

  const scenarios = [buildScenarioIndustrialDense(), buildScenarioVariedCuts()];
  const comparisons = scenarios.map((scenario) => {
    const beforeRow = beforeRaw.scenarios.find((s) => s.scenario === scenario.name);
    const before = beforeRow?.v2 ?? { totalSheets: 0, wasteArea: 0 };
    const after = runScenario(scenario);
    return {
      scenario: scenario.name,
      before: {
        totalSheets: before.totalSheets,
        wasteArea: before.wasteArea,
      },
      after,
      delta: {
  ...
  const report = {
    title: "Fase A — Comparação Nesting cutlayout (antes vs depois)",
    generatedAt: new Date().toISOString(),
    changes: ["A1 ContextoChapa", "A2 janela 128 + amostragem estratificada", "A4 ordenação sandwich", "A5 gap-fill 80k + rotação pocket fill"],
    comparisons,
    summary: {
      ...
      avgElapsedMs: Number(
        (comparisons.reduce((a, c) => a + c.after.elapsedMs, 0) / Math.max(1, comparisons.length)).toFixed(1)
      ),
    },
  };
  ...
  const lines: string[] = [
    ...
    ...comparisons.flatMap((c) => [
      `=== ${c.scenario} ===`,
      `Chapas:     ${c.before.totalSheets} → ${c.after.totalSheets} (Δ ${c.delta.sheets >= 0 ? "-" : "+"}${Math.abs(c.delta.sheets)})`,
      `Desperdício: ${c.before.wasteArea} → ${c.after.wasteArea} mm² (Δ ${c.delta.wastePercent})`,
      `Pockets (grid): ${c.after.pocketsCount} | Pockets ≥150×150: ${c.after.largePockets150}`,
      `Fragmentação: ${c.after.fragmentationScore.toFixed(4)}`,
      `Tempo: ${c.after.elapsedMs} ms | Estratégia: ${c.after.strategy}/${c.after.binHeuristic}`,
      "",
    ]),
    "--- Resumo ---",
    `Redução total chapas: ${report.summary.totalSheetReduction}`,
    `Redução total desperdício: ${report.summary.totalWasteReduction} mm²`,
    `Tempo médio: ${report.summary.avgElapsedMs} ms`,
  ];
